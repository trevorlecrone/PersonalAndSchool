...
int parse(char* filename_, Scene &scene) {
    ...
    if (maxVertices > 5000 || maxNormals > 5000) {
        printf("constructing bounding volume hierarchy\n");
    }
    else {
        printf("scene is simple, skip constructing bounding volume hierarchy\n");
    }
...
Pixel LModel(HitData h, int depth, int max) {
    ...
    if (VERBOSE) {
        fprintf(logPointer, "amb_r: %f, amb_g: %f, amb_b: %f\n", r, g, b);
    }
    for (int l = 0; l < h.s.lights.size(); l++) {
        ...
        if (!shadowed) {
            ...
            if (VERBOSE) {
                fprintf(logPointer, "l_dir : %f, %f, %f\n", l_dir.X(), l_dir.Y(), l_dir.Z());
                fprintf(logPointer, "s_normal : %f, %f, %f\n", normal.Normalize().X(), normal.Normalize().Y(), normal.Normalize().Z());
                fprintf(logPointer, "reflected : %f, %f, %f\n", reflected.X(), reflected.Y(), reflected.Z());
                fprintf(logPointer, "angle_factor: %f\n", angle_factor);
                fprintf(logPointer, "intensity : %f, %f, %f\n", l_intense.X(), l_intense.Y(), l_intense.Z());
            }
            if (angle_factor > 0) {
            ...
            if (VERBOSE) {
                fprintf(logPointer, "diff_r: %f, diff_g: %f, diff_b: %f\n", r, g, b);
                fprintf(logPointer, "spec_angle %f\n", spec_angle);
                fprintf(logPointer, "pow(spec_angle, mat_phong) %f\n", pow(spec_angle, mat_phong));
            }
        }
    }
    ...
    if (VERBOSE) {
        fprintf(logPointer, "r: %f, g: %f, b: %f\n", r, g, b);
        fprintf(logPointer, "p_r: %d, p_g, %d, p_b %d \n\n", f_color.r, f_color.g, f_color.b);
    }
...
Pixel EvaluateRayTree(Scene scene, Ray ray, int depth, int max) {
    ...
    if (nearest_t < 2000) {
        ...
        if (VERBOSE) {
            fprintf(logPointer, "direction vector: [%f, %f, %f]\n", ray.GetDir().X(), ray.GetDir().Y(), ray.GetDir().Z());
            fprintf(logPointer, "i_point: %f %f %f\n", i_point.X(), i_point.Y(), i_point.Z());
            fprintf(logPointer, "t %f\n", nearest_t);
        }
    ...
    if (p_success == 1) {
        ...
        for (int y = 0; y < scene.height; ++y) {
            for (int x = 0; x < scene.width; ++x) {
                ...
                if (VERBOSE) {
                    fprintf(logPointer, "____________________________________");
                    fprintf(logPointer, "x,y: %d, %d\n", x, y);
                }
